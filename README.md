...
- **Godot thin client (#109 / #118 / #141)** — native UI talking to the same `/ws` server (not a rewrite). **3D street is the Steam presentation goal**; ASCII FPV remains a toggle. [godot_client/](godot_client/) · [docs/godot-3d.md](docs/godot-3d.md) · [docs/godot-client.md](docs/godot-client.md).
- **Steam quality bar (#130)** — north star: Steam-marketable **3D** Godot product (not a web wrap); comps + gap analysis + ship priority. See [docs/steam-quality-bar.md](docs/steam-quality-bar.md).
